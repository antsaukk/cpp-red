...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) {
    for(auto it = begin; it != end; it++) {
      airports[static_cast<uint32_t>(*it)]++;
    }
  }
  ...
  static const size_t SZ = static_cast<size_t>(TAirport::Last_);
  using Item = pair<TAirport, size_t>;
  using Items = array<Item, SZ>;
  ...
  Items GetItems() const {
    ...
    for(size_t i = 0; i < airports.size(); i++) {
      res[i] = make_pair(static_cast<TAirport>(i), airports[i]);
    }
    return res;
  }

private:
  array<size_t, SZ> airports = {};
};
...
void TestManyGetItems() {
  ...
  for (int step = 0; step < 100'000'000; ++step) {
    ...
    for (const auto count : counter.GetItems()) {
      total += count.second;
    }
  }
  // Assert to use variable total so that compiler doesn't optimize it out
